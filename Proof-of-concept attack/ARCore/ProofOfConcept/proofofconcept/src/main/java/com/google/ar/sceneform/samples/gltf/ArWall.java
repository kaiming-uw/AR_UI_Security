...
public class ArWall {
    ...
    public ArWall(String name, Color wallColor, Context context, ArFragment arFragment, AnchorNode parent,
                  TextView infoView, Vector3 offset, boolean disableRender, boolean disableCollider, boolean transparentTexture) {
        this.name = name;
        this.numTaps = 0;
...
//        infoView.setText(name + ", Clicked " + numTaps);
        createCubeRemoveOnTap(context, arFragment, wallColor, offset, transparentTexture);
    }
     ...
     * @param offset offset from parent anchor position
     */
    private void createCubeRemoveOnTap(Context context, ArFragment arFragment, Color wallColor, Vector3 offset, boolean transparentTexture) {
        if (!transparentTexture)
        {
            MaterialFactory.makeTransparentWithColor(context, wallColor)
                    .thenAccept(material -> {
                        Vector3 vector3 = new Vector3(Globals.CUBE_WIDTH * 16, Globals.CUBE_WIDTH * 16, Globals.CUBE_WIDTH * 0.1f);
...
//                            infoView.setText(updateStateText);
        ...
        {
            Texture.builder().setSource(context, R.drawable.transparent)
                    .build()
                    .thenAccept((texture -> {
