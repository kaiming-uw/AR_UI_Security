...
public class AnchorUIManagerInvisibility : MonoBehaviour
{
    /// <summary>
    /// Anchor UI manager singleton instance
    /// </summary>
    public static AnchorUIManagerInvisibility Instance;
    ...
    private Anchor Cube1, Cube2, Cube3_1, Cube3_2, Cube3_3, Cube3_4;

    /***
    The hard part is getting a handle to the other script. 
    We import the objects in the game scene after draging the object 
    ***/
    private CompAInvisibility ComponentA;
    private CompBInvisibility ComponentB;
    ...
    private void PlaceAnchor()
    {
        ...
        if (expNum == 0){
            ...
            ComponentA = GameObject.FindObjectOfType(typeof(CompAInvisibility)) as CompAInvisibility;
        ...
        if (expNum == 1){
            ...
            ComponentB = GameObject.FindObjectOfType(typeof(CompBInvisibility)) as CompBInvisibility;
            Cube2 = (Anchor)Instantiate(ComponentB.launch(), _secondAnchorTransform.position - _raycastOrigin.TransformDirection(Vector3.forward) * 0.5f, _secondAnchorTransform.rotation);
            Cube2.name = "Cube2";
        }
        if (expNum == 2){
            Cube2.OnHideButtonPressed();
            if (ComponentB.next(expNum)!=null){
                Cube3_1 = (Anchor)Instantiate(ComponentB.next(expNum), _secondAnchorTransform.position - _raycastOrigin.TransformDirection(Vector3.forward) * 0.5f, _secondAnchorTransform.rotation);
                Cube3_1.name = "Cube3_1";
            }

        }
        if (expNum == 3){
            Cube3_1.OnHideButtonPressed();
            if (ComponentB.next(expNum)!=null){
                Cube3_2 = (Anchor)Instantiate(ComponentB.next(expNum), _secondAnchorTransform.position - _raycastOrigin.TransformDirection(Vector3.forward) * 0.5f, _secondAnchorTransform.rotation);
                Cube3_2.name = "Cube3_2";
            }

        }
        if (expNum == 4){
            Cube3_2.OnHideButtonPressed();
            if (ComponentB.next(expNum)!=null){
                Cube3_3 = (Anchor)Instantiate(ComponentB.next(expNum), _secondAnchorTransform.position - _raycastOrigin.TransformDirection(Vector3.forward) * 0.5f, _secondAnchorTransform.rotation);
                Cube3_3.name = "Cube3_3";
            }

        }
        if (expNum == 5){
            Cube3_3.OnHideButtonPressed();
            if (ComponentB.next(expNum)!=null){
                Cube3_4 = (Anchor)Instantiate(ComponentB.next(expNum), _secondAnchorTransform.position - _raycastOrigin.TransformDirection(Vector3.forward) * 0.5f, _secondAnchorTransform.rotation);
                Cube3_4.name = "Cube3_4";
            }
        }
