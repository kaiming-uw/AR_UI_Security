...
class App {
  ...
  onTouchEnd = (event) =>
  {
    ...
    if (intersects.length > 0) {
      ...
      this.cubeInfoAreas[mapping[String(currObj.name)]].textContent = makeClickText(currObj.name, currObj.clickCount);
  ...
  onNextClick = (event) =>
  {
    ...
    else if (this.experimentNum == 2) 
    {
      ...
      document.getElementById("nextButton").textContent="Experiment: " + (this.experimentNum-2);
      this.displayToUser("After verifying blocking cube, begin experiments");
      this.displayToUser("Click Experiment: " + (this.experimentNum-2) + " to proceed.")
      // document.getElementById("nextButton").style.visibility = 'hidden';

    } 
    else if (this.experimentNum > 2 && this.experimentNum <= 6)
    {
      
      this.removeObjectByName(index_mapping[this.experimentNum])
      // return MakeCube and retrieve the first cube 
      let cubes = this.compBObject.next(this.experimentNum - 3);
      let cube3 = cubes[0]

      // let previousCube = this.anchoredObjects.pop();
      this.anchoredObjects.push(cube3)
      
      document.getElementById("nextButton").textContent="Experiment: " + (this.experimentNum + 1 - 3);
      this.displayToUser("After testing invisibility experiment, begin next experiment");
      this.displayToUser("Click Experiment: " + (this.experimentNum + 1 - 3) + " to proceed.")
    }
    else if (this.experimentNum > 6)
    {
      this.removeObjectByName("3.3")
